...
const Usuario = (props) => {
    ...
    useEffect(() => {
        ...
        async function fetchAll() {
            const results = await Promise.all(props.usuario.peliculasLikeadas.map((id) => fetch(`https://api.themoviedb.org/3/movie/${id}?api_key=43fd83d3a9756a2f59b0de39480b3bf7&language=es-MX`).then((r) => r.json())));
